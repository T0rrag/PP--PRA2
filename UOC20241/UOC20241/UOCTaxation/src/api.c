...
tApiError api_addTenant(tApiData* data, tCSVEntry entry) {
    ...
    char taxOfficeCode[MAX_OFFICE_CODE + 1];
    ...
    strncpy(cadastralRef, tenant.cadastral_ref, MAX_CADASTRAL_REF);
    rent = tenant.rent;

    /*EXTRACT TAXOFFICE CODE*/
    csv_getAsString(entry, 7, taxOfficeCode, MAX_OFFICE_CODE + 1);
    ...
    /*TAX OFFICE FIND PROCEDURE*/
    office = taxOfficeList_find(&(data->taxOffices), taxOfficeCode);
    if (office == NULL) {
        /*ADD NEW OFFICE*/
        taxOfficeList_insert(&(data->taxOffices), taxOfficeCode);
        office = taxOfficeList_find(&(data->taxOffices), taxOfficeCode);
    }

   /*DATA PRICES UPDATE*/
    if (office != NULL) {
        invoiceList_update(&(office->rentInvoices), startDate, endDate, cadastralRef, rent);
    } 
